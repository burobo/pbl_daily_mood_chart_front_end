            ...
            <div class="modal-content">
                ...
                <div class="modal-body p-4">
                        ...
                        <div class="d-flex aligh-item-center justify-content-center">
                            <div>
                                <input class="btn btn-outline-secondary btn-lg"
                                    :class="{ 'btn-primary': isMoodSelected(0) }" type="button" value="😢"
                                    @click="selectMood(0)" />
                                <input class="btn btn-outline-secondary btn-lg"
                                    :class="{ 'btn-primary': isMoodSelected(1) }" type="button" value="🙁"
                                    @click="selectMood(1)" />
                                <input class="btn btn-outline-secondary btn-lg"
                                    :class="{ 'btn-primary': isMoodSelected(2) }" type="button" value="😐"
                                    @click="selectMood(2)" />
                                <input class="btn btn-outline-secondary btn-lg"
                                    :class="{ 'btn-primary': isMoodSelected(3) }" type="button" value="😃"
                                    @click="selectMood(3)" />
                                <input class="btn btn-outline-secondary btn-lg"
                                    :class="{ 'btn-primary': isMoodSelected(4) }" type="button" value="😄"
                                    @click="selectMood(4)" />
                            </div>
                        </div>
                    </div>
                    <div class="mb-3">
                        ...
                        <div class="row" v-for="(sleepRecordRef, idx) in sleepRecoredsRefWrap.sleepRecordsRef.value"
                            :key="idx">
                            <div class="col">
                                <input type="datetime-local"
                                    v-model="sleepRecoredsRefWrap.sleepRecordsRef.value[idx].sleep_start_time" />
                            </div>
                            <div class="col">
                                <input type="datetime-local"
...
class SleepRecord {
    constructor(sleepStartTime, sleepEndTime) {
        this.sleep_start_time = this.format(sleepStartTime || selectedDateRef.value);
        this.sleep_end_time = this.format(sleepEndTime || selectedDateRef.value);
    }
    format(date) {
        return `${date.getFullYear()}-${zeroPadding(2, date.getMonth() + 1)}-${zeroPadding(2, date.getDate())} ${zeroPadding(2, date.getHours())}:${zeroPadding(2, date.getMinutes())}`
    }
    sleepMinutes() {
        return (new Date(this.sleep_end_time.replace(' ', 'T')).getTime() - new Date(this.sleep_start_time.replace(' ', 'T')).getTime()) / 1000 / 60;
    }
}
...
// v-forを使うためにwrapが必要
const sleepRecoredsRefWrap = { sleepRecordsRef }
const memoRef = ref("")
...
async function upsertMood() {
    const { data: upsertMoodData, pending: upsertMoodPending, error: upsertMoodError, refresh: upsertMoodRefresh } = useFetch(
        "/moods",
        {
            method: "POST",
            baseURL: config.public.API_PROXY_BASE_URL,
            ...
            async onRequest({ request, options }) {
                ...
                options.body.date = `${selectedDateRef.value.getFullYear()}${zeroPadding(2, selectedDateRef.value.getMonth() + 1)}${zeroPadding(2, selectedDateRef.value.getDate())}`;
...
async function fetchDailyMood() {
    const { data: dailyMoodData, pending: dailyMoodPending, error: dailyMoodError, refresh: dailyMoodRefresh } = await useFetch(
        "/daily_logs",
        {
            method: "POST",
            baseURL: config.public.API_PROXY_BASE_URL,
